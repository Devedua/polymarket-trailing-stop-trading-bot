...
use polymarket_client_sdk::clob::types::{
    Amount, AssetType, OrderType, Side, SignatureType as PmSignatureType,
};
use polymarket_client_sdk::clob::{Client as PmClobClient, Config as PmClobConfig};
use polymarket_client_sdk::{POLYGON, contract_config as pm_contract_config};
use uuid::Uuid;
...
type PmAuthedClob = PmClobClient<Authenticated>;
...
/// Polygon chain ID (137), same as [`POLYGON`].
fn polygon() -> u64 {
    POLYGON
}
...
pub struct PolymarketApi {
    ...
    /// Authenticated Polymarket CLOB client from `polymarket-client-sdk` (orders, balances).
    pm_clob: Arc<tokio::sync::Mutex<Option<PmClobState>>>,
}
...
impl PolymarketApi {
    pub fn new(
        ...
        Self {
            ...
            pm_clob: Arc::new(tokio::sync::Mutex::new(None)),
        }
    }

    fn pm_signature_type(&self) -> Result<PmSignatureType> {
        ...
        Ok(match sig_type {
            0 => PmSignatureType::Eoa,
            1 => PmSignatureType::Proxy,
            2 => PmSignatureType::GnosisSafe,
            _ => anyhow::bail!("Invalid signature_type: {}", sig_type),
        })
    }

    /// Ensure authenticated CLOB client exists (polymarket-client-sdk). Call after `authenticate()` or when placing orders / checking balance.
    async fn ensure_clob_client(&self) -> Result<(PmAuthedClob, Arc<LocalSigner>)> {
        let mut guard = self.pm_clob.lock().await;
        if let Some(state) = guard.as_ref() {
            return Ok((state.client.clone(), state.signer.clone()));
        }
        ...
        let credentials = Credentials::new(
            Uuid::parse_str(api_key.trim()).context("api_key must be a valid UUID")?,
            api_secret.clone(),
            api_passphrase.clone(),
        );

        let base = PmClobClient::new(self.clob_url.trim(), PmClobConfig::default())
            .map_err(|e| anyhow::anyhow!("{}", e))?;
        let pm_sig = self.pm_signature_type()?;

        let client = match (self.proxy_wallet_address.as_ref(), pm_sig) {
            (Some(proxy), PmSignatureType::Proxy | PmSignatureType::GnosisSafe) => {
                let funder = AlloyAddress::parse_checksummed(proxy, None)
                    .with_context(|| format!("Invalid proxy_wallet_address: {}", proxy))?;
                base
                    .authentication_builder(signer.as_ref())
                    .credentials(credentials)
                    .funder(funder)
                    .signature_type(pm_sig)
                    .authenticate()
                    .await
            }
            (None, PmSignatureType::Eoa) => {
                base
                    .authentication_builder(signer.as_ref())
                    .credentials(credentials)
                    .signature_type(PmSignatureType::Eoa)
                    .authenticate()
                    .await
            }
            (None, PmSignatureType::Proxy | PmSignatureType::GnosisSafe) => {
                anyhow::bail!("signature_type requires proxy_wallet_address (funder) for proxy/safe mode");
            }
            (Some(_), PmSignatureType::Eoa) => {
                anyhow::bail!("proxy_wallet_address is set but signature_type is EOA; use 1 or 2");
            }
        }
        .map_err(|e| anyhow::anyhow!("{}", e))?;

        *guard = Some(PmClobState {
            client: client.clone(),
            signer: signer.clone(),
        });
        Ok((client, signer))
    ...
    pub async fn authenticate(&self) -> Result<()> {
        ...
        *self.authenticated.lock().await = true;
        eprintln!("✅ Successfully authenticated with Polymarket CLOB API (polymarket-client-sdk)");
    ...
    /// Place a limit order via polymarket-client-sdk (build → sign → post).
    ...
    pub async fn place_order(&self, order: &OrderRequest) -> Result<OrderResponse> {
        ...
        let (clob, signer) = self.ensure_clob_client().await?;
        ...
        let price = rust_decimal::Decimal::from_str(order.price.trim()).context("parse limit price")?;
        let size = rust_decimal::Decimal::from_str(order.size.trim()).context("parse limit size")?;
        let signable = clob
            .limit_order()
        ...
        let first = posted
            .pop()
            .ok_or_else(|| anyhow::anyhow!("empty response from post_order"))?;
    ...
    /// Place multiple limit orders (one post per order via polymarket-client-sdk).
    pub async fn place_limit_orders(&self, orders: &[OrderRequest]) -> Result<Vec<OrderResponse>> {
        ...
        for (i, order) in orders.iter().enumerate() {
            ...
            let price = match rust_decimal::Decimal::from_str(order.price.trim()) {
                Ok(p) => p,
                Err(e) => {
                    out.push(OrderResponse {
                        order_id: None,
                        status: "REJECTED".to_string(),
                        message: Some(format!("Invalid price: {}", e)),
                    });
                    continue;
                }
            };
            let size = match rust_decimal::Decimal::from_str(order.size.trim()) {
                Ok(s) => s,
                Err(e) => {
                    out.push(OrderResponse {
                        order_id: None,
                        status: "REJECTED".to_string(),
                        message: Some(format!("Invalid size: {}", e)),
                    });
                    continue;
                }
            };
            let one: Result<String, anyhow::Error> = async {
                let signable = clob
                    .limit_order()
                    .token_id(order.token_id.as_str())
                    .side(side)
                ...
                let first = posted
                    .pop()
                    .ok_or_else(|| anyhow::anyhow!("empty post_order response"))?;
                Ok(first.order_id)
            }
            .await;
            match one {
    ...
    /// Cancel a specific order by order id (CLOB). Uses REST DELETE with HMAC auth.
    ...
    /// Check USDC balance and allowance for buying tokens (CLOB balance API).
    ...
    pub async fn check_usdc_balance_allowance(&self) -> Result<(rust_decimal::Decimal, rust_decimal::Decimal)> {
        let (clob, _) = self.ensure_clob_client().await?;
        let sig = self.pm_signature_type()?;
    ...
    /// Check token balance only (for redemption/portfolio scanning) via CLOB balance API.
    ...
    /// Check token balance and allowance before selling (CLOB balance API).
    ...
    /// Refresh cached allowance for outcome token before selling (CLOB update-balance API).
    pub async fn update_balance_allowance_for_sell(&self, token_id: &str) -> Result<()> {
        ...
        let req = UpdateBalanceAllowanceRequest::builder()
            ...
            .build();
        clob
            .update_balance_allowance(&req)
            .await
            .map_err(|e| anyhow::anyhow!("{}", e))
    }

    /// Get the CLOB contract address for Polygon (from polymarket-client-sdk contract config).
    fn get_clob_contract_address(&self) -> Result<String> {
        let config = pm_contract_config(polygon(), false)
            .ok_or_else(|| anyhow::anyhow!("Contract config not available for chain"))?;
        Ok(format!("{:#x}", config.exchange))
    }

    /// Get the CTF contract address for Polygon (from polymarket-client-sdk contract config).
    ...
    pub async fn check_all_approvals(&self) -> Result<Vec<(String, bool, bool)>> {
        ...
        let config = pm_contract_config(polygon(), false)
            .ok_or_else(|| anyhow::anyhow!("Contract config not available"))?;
        let neg_risk_config = pm_contract_config(polygon(), true)
    ...
    pub async fn place_market_order(
        ...
        let (clob, signer) = self.ensure_clob_client().await?;
        let side_sdk = if is_buy { Side::Buy } else { Side::Sell };
        let amt_dec = rust_decimal::Decimal::from_str(&amount_str).context("parse market order amount")?;
        let amount_sdk = if is_buy {
            Amount::usdc(amt_dec).map_err(|e| anyhow::anyhow!("{}", e))?
        } else {
            Amount::shares(amt_dec).map_err(|e| anyhow::anyhow!("{}", e))?
        };
        let ot_sdk = match ot {
            "FOK" => OrderType::FOK,
            "FAK" => OrderType::FAK,
        ...
        for attempt in 1..=max_retries {
            let post_one = async {
                ...
                let first = posted
                    .pop()
                    .ok_or_else(|| anyhow::anyhow!("empty post_order response for market order"))?;
                Ok::<String, anyhow::Error>(first.order_id)
            };
            match post_one.await {
                Ok(order_id) => {
    ...
    /// Place multiple market orders (one build/sign/post per order via polymarket-client-sdk).
    pub async fn place_market_orders(
        ...
        for (i, &(token_id, amount, side, order_type)) in orders.iter().enumerate() {
            ...
            let is_buy = side == "BUY";
            let side_sdk = if is_buy { Side::Buy } else { Side::Sell };
            let amt_dec = match rust_decimal::Decimal::from_str(&amount_str) {
                Ok(d) => d,
                Err(e) => {
                    out.push(OrderResponse {
                        order_id: None,
                        status: "REJECTED".to_string(),
                        message: Some(format!("Invalid amount: {}", e)),
                    });
                    continue;
                }
            };
            let amount_sdk = match if is_buy {
                Amount::usdc(amt_dec)
            } else {
                Amount::shares(amt_dec)
            } {
                Ok(a) => a,
                Err(e) => {
                    out.push(OrderResponse {
            ...
            let ot_sdk = match ot {
                ...
                _ => {
                    out.push(OrderResponse {
                        order_id: None,
                        status: "REJECTED".to_string(),
                        message: Some(format!("Invalid order_type: {}", ot)),
                    });
                    continue;
                }
            };
